...
import {
  HardDrive, Wifi, WifiOff, RefreshCw, Plus, Copy, Eye, EyeOff,
  CheckCircle, AlertTriangle, Clock, Trash2, Pencil, Save, X, Loader2,
} from "lucide-react"
...
export default function DevicesPage() {
  ...
  const [compartmentsMap, setCompartmentsMap] = useState<Record<string, Compartment[]>>({})
  ...
  const load = useCallback(async () => {
    ...
    const { data: devs, error } = await supabase
        .from("devices")
        .select("id, label, device_key, firmware, is_online, last_seen_at, created_at")
        .eq("owner_id", user.id)
        .order("created_at")
  ...
  const addDevice = async () => {
    if (!userId) return
    if (!newLabel.trim()) { toast.error("Please give your device a label"); return }
    setSaving(true)
  ...
  const saveLabel = async () => {
    ...
    const { error } = await supabase
        .from("devices").update({ label: editLabel.trim() }).eq("id", editDevice.id)
    if (error) toast.error(error.message)
    else { toast.success("Label updated"); setEditDevice(null); load() }
  }

  // ── Delete device ───────────────────────────────────────────────────────────

  const deleteDevice = async (id: string) => {
    if (!confirm("Remove this device? All compartment and schedule data will be deleted.")) return
    const { error } = await supabase.from("devices").delete().eq("id", id)
    if (error) toast.error(error.message)
    else { toast.success("Device removed"); load() }
  }
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              Manage your ESP32 dispensers. Each device has a unique key used for API authentication.
            </p>
          </div>
          <div className="flex gap-2">
            ...
            <Button size="sm" onClick={() => { setNewKey(generateKey()); setShowAdd(true) }}>
              ...
              <CardContent className="flex flex-col items-center gap-4 py-16 text-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground max-w-xs mx-auto mt-1">
                    Add your ESP32 dispenser to start automating medication schedules.
                  </p>
                </div>
                <Button onClick={() => { setNewKey(generateKey()); setShowAdd(true) }}>
                      ...
                      <CardContent className="p-5">
                        <div className="flex flex-wrap items-start gap-4">
                          ...
                          <div className="flex-1 min-w-0 space-y-2">
                            <div className="flex flex-wrap items-center gap-2">
                              <h2 className="font-semibold text-lg">{device.label ?? "Unnamed Device"}</h2>
                              <Badge className={device.is_online ? "bg-green-500" : "bg-muted text-muted-foreground"}>
                                {device.is_online ? "Online" : "Offline"}
                              </Badge>
                              {device.firmware && (
                                  <Badge variant="outline" className="text-xs">{device.firmware}</Badge>
                              )}
                            </div>
                        ...
                        <span className="flex items-center gap-1">
                          ...
                          Last seen: {relativeTime(device.last_seen_at)}
                              ...
                              <span>ID: <code className="text-xs">{device.id.slice(0, 8)}…</code></span>
                            </div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              Flash this key into the ESP32 as <code>DEVICE_KEY</code>. Keep it secret.
                            </p>

                            {/* Compartments mini-view */}
                            {comps.length > 0 && (
                                <div className="flex gap-2 pt-1">
                                  {comps.sort((a, b) => a.slot - b.slot).map(c => (
                                      <div
                                          key={c.slot}
                                          className={cn(
                                              "flex flex-col items-center gap-0.5 rounded-lg border px-3 py-2 text-center text-xs",
                                              c.medication_name ? "bg-background" : "bg-muted/30 border-dashed"
                                          )}
                                      >
                                        <span className="font-semibold">Slot {c.slot}</span>
                                ...
                                {c.medication_name ?? "Empty"}
                                        ...
                                        {c.medication_name && (
                                            <span className={cn(
                                                "font-medium",
                                                c.pill_count / c.capacity < 0.2 ? "text-amber-600" : "text-green-600"
                                            )}>
                                  {c.pill_count} pills
                                </span>
                                        )}
                          ...
                          <div className="flex gap-1">
                            <Button
                                ...
                                onClick={() => { setEditDevice(device); setEditLabel(device.label ?? "") }}
                                title="Rename"
                            >
        ...
        <Card className="bg-muted/40 border-dashed">
          <CardContent className="py-4">
            ...
            <div className="space-y-1 text-xs text-muted-foreground font-mono">
              <p>POST /api/device/heartbeat  — called every 30s (updates online status)</p>
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              All requests require header: <code className="bg-muted px-1 rounded">X-Device-Key: your-key</code>
            </p>
          </CardContent>
        </Card>
        ...
        <Dialog open={showAdd} onOpenChange={setShowAdd}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                Give your dispenser a name and copy the generated key into your ESP32 firmware.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex items-center gap-2 rounded-lg border bg-muted/40 px-3 py-2">
                  ...
                  <button onClick={() => copyKey(newKey)} className="shrink-0 text-muted-foreground hover:text-foreground">
                    <Copy className="size-3.5" />
                  </button>
                </div>
              ...
              <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
                <p className="text-xs text-amber-800 flex items-start gap-2">
                  ...
                  Save this key now — for security it will be masked after registration.
                  You can always generate a new key from the devices list.
                </p>
              </div>
            </div>
